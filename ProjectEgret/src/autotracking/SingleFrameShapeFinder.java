...
public class SingleFrameShapeFinder {
	...
	/**
	 * 
	 * @param emptyBackgroundFrame - the "empty" background frame to compare later
	 *                             frames to
	 * @param brightnessThreshold  - how different from the background must the
	 *                             shape's color be? (0-255)
	 * @param minDetectedShapeArea - how large (in pixels squared) must the shape
	 *                             be?
	 */
	public SingleFrameShapeFinder(Mat emptyBackgroundFrame, double brightnessThreshold, double minDetectedShapeArea,
			double maxDetectedShapeArea) {
